...
def main():
    ...
    ax.imshow(gradient, aspect='auto', cmap=CMAP_NAME,
              origin='lower', extent=[0, 1, PHYSICAL_VMIN, PHYSICAL_VMAX])
    ...
    for low, high, label in SEDIMENT_DATA:
        ...
        if low > VIEW_VMAX: continue
        if high < VIEW_VMIN: continue

        # --- 防重疊邏輯 ---
        text_y = mid
        if mid < VIEW_VMIN: text_y = VIEW_VMIN + 0.5
        if mid > VIEW_VMAX: text_y = VIEW_VMAX - 0.5
