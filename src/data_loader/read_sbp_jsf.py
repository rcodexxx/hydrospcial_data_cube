...
def read_sbp_jsf(jsf_file_path, verbose=False):
    ...
    amps_2d = np.array(
        [np.pad(a, (0, max_len - len(a))) for a in raw["sbp"]["amps"]], dtype=np.float32
    )
    ...
    water_depth = np.where(np.isfinite(altitude_arr), depth_arr + altitude_arr, np.nan)
    ...
    if len(att["time"]) >= 2:
        ...
        cd["heading"] = _interp_heading(t_arr, at, np.array(att["heading"]))
    ...
    if verbose:
        ...
        if valid.sum() > 1:
            ...
            print(
                f"BW             : " f"{cd['end_freq_hz'][0]-cd['start_freq_hz'][0]} Hz"
            )
            print(f"Sweep          : {cd['sweep_ms'][0]:.1f} ms")
            print(f"ns/sample      : {cd['ns_per_sample'][0]} ns")
            print(f"Ping spacing   : mean={spacing.mean():.2f}m")
            print(f"Water depth    : " f"{np.nanmin(wd):.2f}~{np.nanmax(wd):.2f}m")
            print(f"Heave          : " f"{cd['heave_m'].mean():.4f}m")
