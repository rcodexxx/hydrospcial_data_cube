...
def read_sss_jsf(jsf_file_path, verbose=False):
    ...
    for ch in ["LF_port", "LF_stbd", "HF_port", "HF_stbd"]:
        ...
        amps_2d = np.array(
            [np.pad(a, (0, max_len - len(a))) for a in r["amps"]],
            dtype=np.float32)
        ...
        if len(att_fallback["time"]) >= 2:
            ...
            if nan_p.any():
                pitch_arr[nan_p] = _interp_clamp(
                    t_arr[nan_p], at,
                    np.array(att_fallback["pitch"]))[nan_p]
            if nan_r.any():
                roll_arr[nan_r] = _interp_clamp(
                    t_arr[nan_r], at,
                    np.array(att_fallback["roll"]))[nan_r]
            if nan_h.any():
                heading_arr[nan_h] = _interp_heading(
                    t_arr[nan_h], at,
                    np.array(att_fallback["heading"]))[nan_h]
        ...
        std[ch] = {
            ...
            "lat":            lat_arr,
            "lon":            lon_arr,
            "pos_source":     np.where(~np.isnan(lat_arr), 0,
                                       255).astype(np.uint8),
        }
